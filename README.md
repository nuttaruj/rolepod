...
- **Cross-family reviewer** — `rolepod-cross-family` sends the adversarial review / debug consult / plan advisory to a *different model family* in one command: pool from `~/.rolepod/cross-family` (or the project's `.rolepod/cross-family`; `none` disables), first usable member, read-only on **its own default model**, evidence anchored; a member that fails is logged and skipped, all fail → the Lead's own path. The commit gate now insists on it while a pool is usable. → [docs/cli-support.md](docs/cli-support.md#cross-family-externals--one-runner-any-lead)
