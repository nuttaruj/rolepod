...
| [**rolepod-seo**](https://github.com/nuttaruj/rolepod-seo) (v0.2+) | 4 search skills — `/seo-audit` (SEO + GEO + AEO, Quick/Full, scored with evidence), `/seo-fix-plan`, `/seo-schema`, `/seo-page-brief`; skills-only, stdlib collector, no MCP, no hooks | Audit → fix plan hands to uiproof (rendered DOM / CWV), wplab (WordPress meta), `content-strategist` (copy); `content-strategist` stops on technical SEO and routes here |
