...
Verifier per evidence type: `qa-tester` (user-visible E2E / UI; unit-suite failures are the writer's) · `performance-engineer` · `security-engineer` · `devops-sre` (CI / deploy smoke). Brief: change manifest + acceptance criteria + tools; several types → ONE message, same frozen change.
