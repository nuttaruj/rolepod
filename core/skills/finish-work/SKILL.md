...
{{INCLUDE: core/fragments/gates-s1-s5.md}}
...
- A red required lane → the Lead triages it, then briefs the lane's owner: a diff-caused red → the path's owner fixes it, infra → `devops-sre`, an R1-sized fix or no subagents → the Lead; the Lead re-pushes. No per-iteration permission once merge intent is approved. Never merge over a red required lane, and never auto-merge a PR with one.
