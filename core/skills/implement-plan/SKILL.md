...
- **Baseline first:** before the first edit run the task's verify command once on the untouched tree and record what already fails as limitations — the task owner does it for a delegated task, the Lead only for its own R1 work (never both).
...
- Absolute: the task owner NEVER commits and NEVER expands scope. A path nobody in the wave owns → touch it, one `Also touched:` line in the brief; a path another owner holds → stop, `NEEDS: <path> — <one-line change>` in the brief; the Lead applies it at integration (R1-sized) or reassigns.
