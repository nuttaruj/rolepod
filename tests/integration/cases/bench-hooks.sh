...
[ "$rows" -ge 14 ] && echo "  ✓ one row per hook payload ($rows rows)" || { echo "  ✗ only $rows rows"; fail=$((fail+1)); }
...
printf '%s' "$j" | python3 -c 'import json,sys; d=json.load(sys.stdin); assert d["runs"]==1 and len(d["rows"])>=14 and "PreToolUse Edit" in d["per_event_ms"]' 2>/dev/null \
