...
check "hook scripts per manifest = Claude 14 / Codex 9 / Gemini 5 / Cursor 3 / Antigravity 6 (actual: $HC_CLAUDE/$HC_CODEX/$HC_GEMINI/$HC_CURSOR/$HC_AGY)" \
  "[ $HC_CLAUDE -eq 14 ] && [ $HC_CODEX -eq 9 ] && [ $HC_GEMINI -eq 5 ] && [ $HC_CURSOR -eq 3 ] && [ $HC_AGY -eq 6 ]"
