...
- **Slash commands** — see `commands/*.toml`. Use `/rolepod` for high-risk (switches approval mode to plan / read-only).
...
`/rolepod` switches Gemini approval mode to **plan** (read-only) + locks rolepod careful-mode rules:
