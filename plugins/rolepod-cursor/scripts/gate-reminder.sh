...
[ "$HIGH_RISK" -eq 1 ] && MSG="${MSG}HIGH-RISK path edited: $BASE. Fix: security-engineer + ONE general strong pass (the external when the pool is usable, else universal-reviewer) before commit."
emit "${MSG% }" "
import json, os
