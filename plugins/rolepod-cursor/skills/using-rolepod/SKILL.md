...
User explicit ("skip spec" / "just commit" / "answer only" / "no plan" / "ship as-is") overrides the tier. **Verify never fully skips** — `verify-first` is always-on: R1/R2 drop the heavyweight verify (full suite, browser drive), not the lightweight one (R1: the edit tool's echo, no re-read turn; R2: run the checklist's verify command).
