...
| "where should we deepen / refactor for testability / where is the architecture hurting" on the whole repo | **Define (entry)** | tell the user to type /deepen-codebase ($deepen-codebase on Codex) — explicit invoke only; the picked card lands in write-spec | **strong** (the explorer inherits the Lead) |
