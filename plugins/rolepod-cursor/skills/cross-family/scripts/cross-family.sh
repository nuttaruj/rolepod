...
if { [ "$KIND" = "review" ] || [ "$KIND" = "implement" ]; } && [ -z "$JOB_DIR" ] && [ -d "$JOBS" ]; then
  for _ld in "$JOBS"/*-review-*/ "$JOBS"/*-implement-*/; do
    ...
    _lid=$(basename "$_ld")
    _lkind=$(printf '%s' "$_lid" | sed -n 's/^[^-]*-\([a-z]*\)-.*/\1/p'); _lkind=${_lkind:-external}; _lfix="cross-family.sh --collect $_lid (waits)"; [ "$_lkind" = implement ] && _lfix="cross-family.sh --collect $_lid (waits) or --kill $_lid"
...
TREE_EXCLUDE=":(exclude).rolepod/evidence :(exclude).rolepod/session-locks :(exclude).rolepod/parent-active :(exclude).rolepod/gate-bypass.log :(exclude).rolepod/cross-family.asked :(exclude).rolepod/ctx-nudge"
