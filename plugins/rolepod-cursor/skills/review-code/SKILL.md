...
- **Conventions** — the rules the project wrote down (CLAUDE.md, lint / formatter config): import style, naming, error handling, logging, test layout. A written rule the diff breaks is a MAJOR with the rule quoted; a preference nobody wrote down is a MINOR at most. Reviewers get CLAUDE.md injected — cite its line, never restate it.
- **UI** — a11y, hierarchy, consistency, platform conventions if applicable
- **Tests** — strong assertions? Mocks at the right boundary? Race coverage for concurrent code? A diff that *modifies an existing test* on the way to green is a finding until justified — loosened assertion, raised tolerance, deleted case, added skip / only, snapshot updated to absorb the failure. The old test was the contract; changing the test instead of the code needs its own stated why. N call-site tests of one shared rule → collapse to one at the owner plus a smoke each. A new test naming a calendar date or reading the real clock → finding: derive it from one frozen now.
