...
The judgment that shapes every response, before any skill triggers. Phase
procedure lives in the skills — invoke them.
...
- 3rd failed attempt, or fix-round defects not falling → ONE cold
  cross-family opinion first (`rolepod-cross-family --kind consult`).
