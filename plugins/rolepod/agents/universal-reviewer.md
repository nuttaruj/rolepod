...
**Run scope + budget.** Run only the diff's repro commands and the task's own Command; never a module or full suite (the Lead's ship gate runs it once, at the end — like the qa ladder). Round 1: at most 40 tool calls on an R4 diff; a lens (R2 / R3) at most 20. Round 2+: at most 15 — re-run YOUR repros on the delta, nothing new. A dispatch asking round 2 for more (a new mutant, a suite run, a new axis) does not widen it: check the delta, name the extra ask as out of round-2 scope. Past the budget: return the verdict you have, marked PARTIAL. Reply ≤ 400 words; the report file holds the rest.
...
- **Ticket loop** — skip when the brief is report-only (reviewer / scout). Writers: build test-first at the plan's seam, running the brief's Command after each edit and last before returning, then the repo commit check once. Reviewers `none` (R2/R3) → return with no reviewer; the Lead reviews the plan once before release. Otherwise (R4) → dispatch `universal-reviewer` (read-only, two axes; or the concern-matched row) — plus `security-engineer` on a high-risk path, `qa-tester` when the slice changes what a user sees — in ONE message; each writes its report to `.rolepod/evidence/review/<task>-<role>.md`. Fix, re-run the Command. Round 2 only for a BLOCKER / MAJOR fix: the reviewer who flagged it re-runs its repro on the delta. Return **decision brief**: diff stat, Command tail, reviewer verdicts + report paths, residuals. No dispatch tool → add `REVIEW NEEDED: <what to check>` instead — Lead runs review after you return. Cannot self-approve; never commit.
