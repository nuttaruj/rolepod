...
- Re-read the plan artifact if one exists — checkboxes mark the real position, not the
  summary's claim of it; an inline checklist (R2 one file + test / spec-as-plan R3 multi-file) is re-stated in the hand-off with its ticks, or the artifact is written before compacting
