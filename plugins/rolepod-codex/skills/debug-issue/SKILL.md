...
2. Write ONE self-contained prompt to a file — the advisor is cold; it sees only this: the symptom, the repro command, the failed fix attempts with why each failed, and the suspect code inline. Invoke by substituting the file, prefixed `ROLEPOD_BRAIN_SILENT=1` (keeps ambient memory systems from injecting the project narrative into — or capturing — the advisor run; harmless if none installed) — never hand-type code into the argument: `ROLEPOD_BRAIN_SILENT=1 codex exec "$(cat /tmp/consult.md)"` (same pattern for `claude -p` / `gemini -m pro -p`).
