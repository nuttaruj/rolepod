...
- `Owner: <role> · write: external` (pool opt-in, the user's choice per task) — the task owner, in its own worktree, hands the first draft to another CLI: `rolepod-cross-family --kind implement --brief <task-brief> --allow <path>... --detach` (edits outside `--allow` reverted; money / auth / data needs the user's `--allow-risky`), `--collect`s in the FOREGROUND (a background wait leaves the owner idle), then runs its own loop — the writing member never reviews it; the Lead never runs §6 for it, the Lead meanwhile in ANOTHER worktree.
