...
Both stages mandatory for a delegated task that touches a seam (a caller/callee or shared-contract pair), an exported symbol, or more than one production file (its own test file does not count); a delegated single-file, seam-free task skips §6 and is covered by the final whole-implementation review below — which then must be a dispatched reviewer (`universal-reviewer`, or review-code's concern-matched row) holding the cumulative diff plus the acceptance criteria from the spec / the plan's task rows. Lead-executed tasks: §6 collapses to one self-review read cold — run `git diff`, then re-read each touched file from disk (not from memory) before judging (fresh-context simulation).

After all tasks pass per-task review, dispatch a **final whole-implementation review** on the cumulative diff to catch cross-task drift (type/symbol/contract mismatch, unowned files in parallel layouts). Small clean plan (≤2 tasks, disjoint files, per-task reviews clean, no cross-task seam — a caller/callee or shared-contract pair IS a seam) → the Lead's own cold read of the cumulative diff stands in; dispatch the reviewer when cross-task seams exist or any task skipped §6. Hand off to `check-work` only after the final review clears.
