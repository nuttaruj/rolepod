...
# live in README → "Recommended add-ons" — install separately yourself.
# The framework auto-integrates when each is present.
#
# Env vars:
...
#   --target=<cli>    claude|codex|cursor|antigravity|opencode|all (default claude)
...
Choose CLI target:
  ...
  ${BOLD}3${NC}) ${CYAN}cursor${NC}      — Cursor IDE (~/.cursor/)
  ${BOLD}4${NC}) ${CYAN}antigravity${NC} — Antigravity CLI / agy (~/.gemini/)
  ${BOLD}5${NC}) ${CYAN}opencode${NC}    — opencode (~/.config/opencode/)
...
EOF
    ...
    case "${target_choice:-1}" in
      ...
      3|cursor)       INTERACTIVE_TARGET="cursor" ;;
