...
if printf '%s' "$PROMPT" | grep -qiE "$CLAIM_RX"; then
  MSG="🔍 rolepod claim-check: this asks for an analysis/diagnosis/explanation/status about real code or state. Apply Verify-first: READ the primary source (read_file / search_file_content / run the command) and cite file:line — never state it from memory. (off: ROLEPOD_NUDGE_OFF=1)"
  python3 -c "
import json
