...
# (auth/billing/migrations/etc.) get a stronger R4-route nudge.
#
...
if [ "$HIGH_RISK" -eq 1 ]; then
  MSG+=$'\n\nHIGH-RISK PATH DETECTED. Route R4: write-spec → write-plan BEFORE the edit. Before commit: security-engineer + ONE strong pass — external via rolepod-cross-family --kind review when the pool is usable (a different CLI than this Lead), else universal-reviewer.'
fi
