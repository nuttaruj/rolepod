...
Teammates spawn through the **Agent tool** (a named Agent call while the flag is on), so rolepod's PreToolUse hooks fire on every spawn exactly as for subagents — the `workflow-tier-nudge` Agent branch included. Upstream picks a teammate's model in this order (v2.1.251+):
