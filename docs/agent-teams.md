...
`/rolepod-team` spawns a real Claude Code agent team — multi-process teammates with shared task list and direct mailbox messaging. This is the [official agent-teams feature](https://code.claude.com/docs/en/agent-teams), NOT rolepod's old subagent-recipe pattern.

## Modes — graceful fallback by environment

`/rolepod-team` adapts to whatever the environment supports. User doesn't manage flags up front; Lead picks the highest-fidelity mode available and announces what it chose.
...
| Aspect | Default Subagent + Task | Agent team (`/rolepod-team`) |
|---|---|---|
...
```
/rolepod-team "Add Stripe billing — high-risk"
```
...
For phase-scoped parallel work, tell `/rolepod-team` to spawn teammates focused on that phase:

```
/rolepod-team "Just the Build phase — spawn 3 engineers in parallel,
