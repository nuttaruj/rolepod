...
| opencode | JS plugin: session lock, post-compact re-anchor, commit deny, the shared `fix-loop-breaker` behind a translator, route record | no — private-docs deny |
| Gemini CLI | frozen adapter (retired upstream) | — |
