...
- **SOFT line (v2.95.0)**: carries `reviewers since last commit: N`; a logic diff with 0 asks for a qa-tester read of the diff (review-code §1 R2 floor — the author never reviews own logic). Advisory.
...
- **Self-guards**: any per-stage `model:`, an `agentType:` that pins a tier (cheap/balanced role, or a strong role under a strong Lead), or either key written as a variable → silent — a platform `agentType:` (`general-purpose` / `Explore`) pins nothing and no longer buys silence (v2.88.0); specialist writer agents → silent; no JSON / no input / no `session_state.py` → silent; no transcript yet (fresh session's first action) → nudge, never deny.
