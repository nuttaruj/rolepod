...
- **Bypass**: `ROLEPOD_NUDGE_OFF=1` — the early exit at the top of the hook, so it silences everything in this hook (route nudge, auto-resume, context-bloat).
...
- **State pointers (v2.102.0)**: the newest `docs/rolepod/plans/*.md` with unchecked steps (`done / open · next: Task N`) and the last phase-log line — the "read the progress file first" step of a long-running-agent harness, done for the Lead at startup / resume / compact.
...
- **Shell-wrapped commits and one evidence tally (v2.167.0)**: `bash|sh|zsh -c` (any short-flag cluster holding `c`), `$SHELL -c`, `eval` and the known wrappers (`env`, `command`, `timeout`, `sudo`, …) are unwrapped before the commit detector and the `git add … && git commit` reader; nesting past depth 4 counts as a commit. The evidence comes from ONE tally, `session_state.py gate-evidence <dir>`, shared with `gate-reminder.sh` (so "COMMIT WILL BLOCK" prints exactly when the gate denies): the window, reviewer dispatches (a write-mode brief never counts), Workflow reviewers from the spawned sub-agent's meta file (never from script text), the phase-log backstop and every CLI's `dispatch-proof` rows (MAX, never summed), the edit ledger (MAX), and anchored external passes whose raw file sits under `evidence/external/`. Each judged commit appends a `phase: gate` row that `rolepod-ticket log` copies into the task's plan line. Out of scope by design (named in the gate header): deliberate evasion — ANSI-C `$'…'` escapes, a bare `&` after an output command, quote- or backslash-split names, `$VAR` indirection, xargs / find -exec / script files / git aliases.
...
scan's, byte-for-byte (test wins; risk = high-risk path AND code file). The ledger rotates at 512 KB (newest 2000 lines) and
...
| Env | When |
|---|---|
| `ROLEPOD_GATES_SOFT=1` | Iterating on doctrine itself; want warnings instead of hard blocks for one session. Set **permanently** (e.g. in a project's `settings.local.json` `env`) it silences the commit gate and the fleet-tier gate — the only hard checkpoints left — for good; `rolepod-stats` shows every use |
...
tree), `precommit-gate.sh`, `gate-reminder.sh`,
`rolepod-stats`.
...
strong-class tokens off the main plan); the internal strong joins at
review-code's round 2+ internal pass, by doctrine, not by the gate.
...
- **`scripts/cross-family.sh` (all CLIs, not a hook, v2.76.0)** — the cross-family runner the hooks and skills call; `render_evidence_scripts` copies it into every plugin tree's `scripts/` next to `stats.sh`, and the hooks resolve it as `../scripts/cross-family.sh` from their own directory (fallback `~/.rolepod/bin/`). One command = pool from config → first usable different-family CLI, read-only, its default model, clean room → raw output + phase-log line anchored.
...
Drift is structurally impossible: the shared scripts have exactly one source (`hooks/`), and the adapter dir is pinned to its two Codex-specific files.
