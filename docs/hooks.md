...
- **Reuse-ladder nudge (v2.109.0; the existing-file trigger cut v2.163.0)**: the same per-session touched-files registry still tracks whether a file was edited before (worktree collision detection needs it), but the ladder itself now fires only on a `Write` that creates a file (`✂ new file <file>: does it need to exist — extend an existing module first? Then reuse before new logic (codebase → stdlib → platform → installed dep → one line before a helper)` — the scope rules stay where they already reach every CLI, the always-on core and S1-S5) and on every edit of a dependency manifest (`package.json`, `requirements*.txt`, `pyproject.toml`, `go.mod`, `Cargo.toml`, `Gemfile`, `composer.json`, `pubspec.yaml`, gradle / pom, `Podfile`, `mix.exs` → a new dependency is the last rung, justify it). An edit of a file already in the codebase is silent — the always-on core states the same ladder every session already, so the per-edit repeat was redundant. Docs / config / assets / build dirs stay silent; `ROLEPOD_NUDGE_OFF=1` silences it. Why here and not gate-reminder: the registry lives in this hook, and it runs first.
...
- **qa-tester never counts at the commit gate (v2.148.4)**: it verifies what the user sees (E2E / UI); the per-diff floor is one read-only `universal-reviewer` pass, so a qa-tester-only dispatch no longer auto-passes a logic diff or silences the SOFT `0 reviewers` line. The round breaker still counts it as review-shaped activity. Prose templates (`*.md.tmpl`, `*.mdc.tmpl`) are prose for the docs exemption, the emoji scan and the money-term scan alike.
...
- **Post-commit worktree reminder (v2.149.0)**: the same core runs after every `git commit` and lists the rolepod-shaped worktrees left behind — `<root>/.worktrees/*`, `<root>/worktrees/*`, or the sibling a generated task brief creates (`../<root-name>-wt-<task>`); never harness-owned paths, never the current checkout: each tagged `merged`, `unmerged, +N` (commits not in HEAD or in the main checkout's HEAD) or `in use` (a live sibling lock under `~/.rolepod/session-locks/`, or created in the last 30 min — a task owner that has not edited yet), listed merged first, plus prunable entries, then the cleanup order (`git worktree remove`, `git branch -d`, `git worktree prune`). Emitted as `additionalContext` for the model and, on Claude, `systemMessage` so the user sees it without a relay (opencode's `runCore` forwards `additionalContext` only; Codex output keys are unverified) — task-owner worktrees otherwise pile up one per task until the disk is full. Runs after the loop counter wrote its state, under one 1 s deadline for the whole listing (a cut list says so); a failed or interrupted commit never reports; `git -C <dir> commit` lists that repo. No leftovers → silent. Runs wherever this core runs (Claude, Codex, opencode); Cursor / agy have no post-tool channel, finish-work Iron Rule 5 covers them.
