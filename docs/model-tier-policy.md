...
Rolepod ships a cost-aware policy that maps **role + risk → model tier**. Each agent's per-CLI overlay carries only a **`tier:`** (a stable, semantic label) — never a model name. `build/merge-agent.py`'s `TIER_MODELS` resolves tier → model at render time for the CLIs that take a pin (Claude aliases), so a model rename or a new generation is **one edit there**, not 48 across the overlays. Codex takes no model pin at all (below). This table is the human-readable view of that map; the static gate verifies the two never drift.

| Tier | Claude | Codex | Use for |
|---|---|---|---|
| **cheap** | `haiku` | — (no pin; effort only) | docs, PM (feature + commercial), customer-facing copy, marketing, FAQ, ADR drafting, read-only scout sweeps — repeatable structured output, no deep architectural reasoning |
| **balanced** | `sonnet` | — (no pin; effort only) | ALL implementation — high-risk paths included (billing / payments / migrations: the net is the strong Lead at dispatch + the strong adversarial review floor, never the dev's tier), QA test writing — the default working tier |
| **strong** | `opus` | — (no pin; effort only) | architecture, security audit, adversarial code review — wrong judgment costs real money or blocks recovery; reviewer must match implementer depth |
...
- **Antigravity's tier is advisory.** `agy` auto-selects the model per task and does not consume a per-agent API model id — the tier is recorded for consistency only, never enforced (see the Antigravity note below).
...
nothing) collapses apex into strong. The dispatch-log `override` field records
which rung was sent.
...
**Antigravity.** Google retired the standalone Gemini CLI for individual accounts on 2026-06-18 (removed from rolepod in v2.177.0 — no adapter, no `--target=gemini`); the live path is Antigravity (`agy`), which **auto-selects** the model per task and does not consume a per-agent API model id. On `agy` the tier a role's frontmatter carries is advisory, not enforced. Do not pin an explicit model id there until an agy-native per-agent model field is verified — pinning an unverified id would silently break dispatch.
...
| Antigravity | n/a — agy auto-selects the model per task | doctrine — no hook logs agy's pick (removed in v2.176.0) |
