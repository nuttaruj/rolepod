...
| **strong** | `opus` | `gpt-5.6-sol` | `gemini-3-pro-preview` | architecture, security audit, adversarial code review — wrong judgment costs real money or blocks recovery; reviewer must match implementer depth |
...
Change what a whole tier resolves to: edit `TIER_MODELS` in `build/merge-agent.py` (the one map) and re-render — e.g. point Claude `strong` back at `inherit` (pre-v2.104 behaviour: the reviewer follows the Lead, the hook floors it), or bump the Codex line to a new generation. Move a single agent between tiers: edit its `tier:` in the three overlays (or just the one CLI you use).
