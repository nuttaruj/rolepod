...
- **strong on Claude is `opus`, pinned in frontmatter (v2.104.0; `inherit` + a hook-side lift from v2.47.0 to v2.103).** The pin is the only floor that holds everywhere the hook does not run: hooks off or broken, the first action of a session (no Lead turn to read → the old floor logged `floor: missed`), a Workflow `agentType:` call (a hook cannot rewrite a script — CourtBook `technician-payout-review` ran 30 × sonnet), another harness or plugin copy with no hooks, and any file in the project named like a stdlib module (`json.py`, `nt.py`) that used to mute every hook silently (measured 2026-09-08: a stray `/tmp/nt.py` on one machine muted the floor for every hook run from `/tmp`; every hook now runs `python3 -I`). `opus` is the **paid ceiling** of the tier by owner decision: a fable-class Lead keeps its own model, but its strong reviewers run opus — never lifted, because lifting the adversarial pass to fable on every review is the cost the tier policy exists to avoid; apex (below) stays an explicit, triggered override. What the hook still does (`workflow-tier-nudge.sh`): `security-engineer` / `universal-reviewer` / `system-architect` dispatched with no `model` under a **known-low** Lead (haiku / sonnet family) get `model: opus` written into the Agent call (`updatedInput`) — belt and braces for a user-level agent file from before v2.104.0 that still says `inherit`; a strong or unknown Lead is left alone; an explicit `model:` on the call is never rewritten, and the commit gate counts a strong reviewer only when it was not explicitly dispatched low. Result: **role → tier is Lead-independent in both directions** — writers pinned balanced under an opus Lead, reviewers at opus under a sonnet Lead, with or without the hook (live-verified 2026-08-17 for the hook path: lifted reviewer transcript = `claude-opus-5`).
- **The Workflow path is where cost leaks, and it cannot be pinned by frontmatter (v2.48.0).** Workflow `agent()` calls default to the Lead's model and no plugin config changes that default; a hook cannot safely rewrite a script either. So the fleet-tier gate in `workflow-tier-nudge.sh` denies, under a strong-class Lead, a fan-out that is model-less, that pastes ONE balanced tier on every stage, or that runs a judgment stage below the Lead — until the script names a tier per stage (or states `// tier-reason: <why>`). v2.50.0 widened it after four probe tasks under an opus Lead all came back sonnet-on-every-stage: the first gate was passed, not applied. This is the one place doctrine was measured to fail on its own — six fleets in one day at opus/fable with the nudge ignored — and it is exactly the ultracode usage pattern (auto-Workflow under an opus/fable Lead). Under a low-class Lead the gate is silent on cost: inherit is already the cheap choice there — but not on the judge floor: since v2.72.0 a high-risk fleet whose judgment stage carries no strong tier is denied under any Lead, and v2.74.0 closed the two holes that let a sonnet Lead run a 30-agent billing review at sonnet (CourtBook `technician-payout-review`): a strong-role `agentType` rendered `inherit` and counted as strong only under a strong Lead (since v2.104.0 it renders `opus` and is the strong slot under any Lead — and spreads like a model pin: a strong-role `agentType` on a fan-out under a low Lead is `strong-spread`), and the prescribed fix is **one strong slot** (the security reviewer or one final adjudicator at `opus`, every fan-out at sonnet/haiku) — a strong pin on a fan-out / ≥2 stages / a sweep under a low Lead is denied as `strong-spread` and never yields. The commit gate mirrors it: a Workflow `agentType` strong reviewer with no explicit strong `model:` counts as the strong pass under any Lead since v2.104.0 (v2.74.0: only when the Lead was not known-low). **v2.88.0** closed the widest remaining hole in the same family (the last residual — a single pin silencing an otherwise-bare fan-out, the rule being per-script — closed in v2.107.0: fan-out calls are judged per call): `agentType:` counted as a tier choice for ANY agent, so one `agentType:'general-purpose'` — which renders no pin and inherits the Lead — silenced the gate entirely (measured on CourtBook: a research fleet under a sonnet Lead, and a 31-turn billing fleet, both passing with no tier named). Only agents that render a pin count now (the cheap/balanced roles, plus a strong role under any Lead since v2.104.0; an `agentType:` from a variable is trusted like a variable `model:`), and the low-Lead nudge names `rolepod:scout` / `haiku` for sweep stages instead of calling an inherited sweep fine.
...
nothing; Gemini) collapses apex into strong. The dispatch-log `override` field records
which rung was sent.
...
Change what a whole tier resolves to: edit `TIER_MODELS` in `build/merge-agent.py` (the one map) and re-render — e.g. point Claude `strong` back at `inherit` (pre-v2.104 behaviour: the reviewer follows the Lead, the hook floors it). Codex has no line: its model is the user's `default_subagent_model`. Move a single agent between tiers: edit its `tier:` in the three overlays (or just the one CLI you use).
