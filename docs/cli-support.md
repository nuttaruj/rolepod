...
| All shell scripts | `bash -n` clean (install.sh, bootstrap.sh, render.sh, 16 core hook scripts, the codex adapter's subagent-model-log.sh + agent-sync.sh, 5 gemini hook scripts, 3 cursor scripts) |
...
- 3 core hooks (4 registrations): `sessionStart` (project context loader), `preToolUse:Write|Edit|MultiEdit` (gate-reminder deny path — a high-risk NEW file without prior verification), `postToolUse:Write|Edit|MultiEdit` (gate-reminder soft reminders — schema-bound file written / high-risk path edited — as `additional_context`, because Cursor feeds `agent_message` to the model only on deny), `beforeShellExecution:git commit` (precommit-gate). Hook JSON I/O follows [cursor.com/docs/hooks](https://cursor.com/docs/hooks): stdin JSON with `hook_event_name`/`tool_name`/`tool_input`/`command`/`workspace_roots` (no `cwd`), stdout JSON with `permission`/`user_message`/`agent_message`/`additional_context`. Exit code 2 = deny. Hook commands run with cwd = plugin root (`./scripts/x.sh` resolves).
