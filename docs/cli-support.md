...
| Ticket loop (v2.144.0) | full — the Owner-line role builds on the Check, dispatches its own reviewers (nested Agent), fixes, returns a decision brief; task owners run in parallel worktrees; the Lead integrates | doctrine — the role runs the loop; no nested dispatch → `REVIEW NEEDED:` in the brief and the Lead runs the review | doctrine — the role runs the loop; no nested dispatch → `REVIEW NEEDED:` in the brief and the Lead runs the review | doctrine — the role runs the loop; no nested dispatch → `REVIEW NEEDED:` in the brief and the Lead runs the review | doctrine — the role runs the loop; no nested dispatch → `REVIEW NEEDED:` in the brief and the Lead runs the review | doctrine — the role runs the loop; no nested dispatch → `REVIEW NEEDED:` in the brief and the Lead runs the review |
