...
| Skills (`<plugin>/skills/<name>/SKILL.md`) | 12 — Core 10 + 2 commands (native) | 12 — Core 10 + 2 commands (native) | 12 — Core 10 + 2 commands (native) | 12 — Core 10 + 2 commands (native; frontmatter stripped to `name` + `description` per Cursor spec) | 12 — Core 10 + 2 commands (native) | 12 — Core 10 + 2 commands (native `SKILL.md`) |
...
| Ticket loop (v2.144.0) | full — the Owner-line role builds, runs the Command, dispatches its own reviewers (nested Agent), fixes, returns a decision brief; task owners run in parallel worktrees; the Lead integrates | doctrine — the role runs the loop; no nested dispatch → `REVIEW NEEDED:` in the brief and the Lead runs the review | doctrine — the role runs the loop; no nested dispatch → `REVIEW NEEDED:` in the brief and the Lead runs the review | doctrine — the role runs the loop; no nested dispatch → `REVIEW NEEDED:` in the brief and the Lead runs the review | doctrine — the role runs the loop; no nested dispatch → `REVIEW NEEDED:` in the brief and the Lead runs the review | doctrine — the role runs the loop; no nested dispatch → `REVIEW NEEDED:` in the brief and the Lead runs the review |
...
| Bash-write-is-an-edit (`block-subagent-commit.sh` write rule, 2026-09-18) | full — PreToolUse `Bash`: ledger row for Lead + sub-agent, write-scope class deny for a sub-agent (`subagent-write-scope.sh` bundled) | full — PreToolUse `Bash`: ledger row for Lead + sub-agent, write-scope class deny for a sub-agent (`subagent-write-scope.sh` bundled, 2026-09-18) — Codex has no Edit/Write/MultiEdit/NotebookEdit tools of its own, so the class deny fires only through this Bash rule | doctrine only — no PreToolUse Bash pre-hook | doctrine only — `beforeShellExecution` has no informational/deny channel for this | doctrine only — accepts no context field on any hook | doctrine only — `tool.execute.before` for Bash is not wired to this rule yet |
...
└── antigravity/
    ...
    └── hooks/hooks.json                 (`{"rolepod": {PreInvocation / PreToolUse(run_command) / Stop}}` — 4 agy-native scripts + the shared precommit-gate)
```
...
Per-CLI hook counts (distinct scripts): Claude registers 16 core hook scripts via the plugin manifest (22 registrations — `session-lifecycle.sh` registers twice, `--lock`/`--unlock`, `subagent-write-scope.sh` twice, Edit/Write and NotebookEdit, `sweep-nudge.sh` five times — UserPromptSubmit reset, both PreToolUse edit matchers, PostToolUse dispatch + read tools; includes the Workflow/Agent tier nudge + dispatch auto-log pair, the `fix-loop-breaker` PostToolUse fail counter and the `subagent-write-scope` per-role write deny). Codex registers 10 in `hooks/hooks.json` (fires natively on Codex ≥0.144; plugin-bundled hooks must be trusted by the user before they fire; `agent-sync.sh` is Codex-only — SessionStart sync of the bundled agents + AGENTS.md block into `~/.codex`, v2.75.0). Gemini registers 5 in `hooks/hooks.json`. Cursor registers 6 in `hooks/hooks.json` (10 registrations — `dispatch-log.sh` on `preToolUse` Task writes the reviewer dispatch-proof line the shared gate counts; `stop-unlock.sh` also records the turn's route line from the Cursor transcript; `gate-reminder.sh` sits on `preToolUse` for the deny path and on `postToolUse` for the soft reminders, because Cursor drops `agent_message` on an allowed action; `sweep-nudge.sh` is the shared Claude script behind a translator on `beforeSubmitPrompt` / `preToolUse` / `postToolUse` / `afterShellExecution`, counting Read bytes from Cursor's `content_length`; `stop-unlock.sh` releases the `cursor-<conversation_id>` session lock; fix-loop-breaker and push-ref-check are not ported — `afterShellExecution` exposes no exit code and `beforeShellExecution` no informational channel; always-on judgment uses an `alwaysApply` rule instead of a SessionStart hook, so the loader script is folded into the rule). Antigravity registers 4 in `hooks.json` at the plugin root under a `rolepod` name key — `session-start` + `model-log` on PreInvocation, `pre-tool` on PreToolUse(`run_command`) which feeds the shared `precommit-gate.sh` and returns its deny as `{decision, reason}`, `stop-unlock` on Stop; agy accepts no context field on any event, so reminders do not exist there and only the transcript-free deny paths (private docs staged, review-round breaker, `ROLEPOD_GATES_HARD=1`) fire until the evidence store is CLI-neutral (contract measured live 2026-09-16 — the previous flat manifest never parsed on agy ≥1.1). The `claim-verify-nudge` answer-path hook ships on Claude / Codex / Gemini / Antigravity but not Cursor — Cursor's `beforeSubmitPrompt` cannot inject pre-answer context. Rolepod ships no add-on hooks — rolepod-brain and GitNexus integrate via their own plugins/CLI.
...
| All shell scripts | `bash -n` clean (install.sh, bootstrap.sh, render.sh, 16 core hook scripts, the codex adapter's subagent-model-log.sh + agent-sync.sh, 5 gemini hook scripts, 6 cursor scripts) |
...
| Codex CLI   | ✓ | ✓ | ✓ native — hooks fire without any opt-in on current Codex; `codex features list` (0.144.1, 2026-07-30) shows `hooks stable true` and the legacy `plugin_hooks` flag `removed` | ✓ verified (15 agents + 12 skills via native loader) | **Production** |
| Gemini CLI  | ✓ | ✓ | ✓ verified (SessionStart hook fires) | ✓ verified (12 skills enumerated) | **Production** |
...
- `gemini skills list` enumerates all 12 rolepod skills (Core 10 + the `rolepod-full` and `deepen-codebase` commands) from `~/.gemini/extensions/rolepod/skills/`.
...
- **Fan-out tier (verified 2026-09-03, codex 0.147.0, official subagent docs):** `model_reasoning_effort` accepts `low` / `medium` / `high` / `xhigh` / `max` / `ultra`; `ultra` = deepest reasoning **and** proactive delegation (the agent spawns its own children; how many is the model's call per task — rolepod never prescribes a count). Un-pinned children resolve explicit spawn → `agents.default_subagent_model` / `agents.default_subagent_reasoning_effort` → the parent; a rolepod role file's own pin always wins. rolepod pins security-engineer at `xhigh` — the effort ceiling on every role and every CLI (v2.75.0; it was `ultra` ≤ v2.73 — a strong × N fan-out per dispatch — and `max` in v2.74) and recommends `[agents] default_subagent_model = "gpt-5.6-terra"` so Ultra's fan-out lands balanced — `make doctor` flags the key. Per-spawn override still exists in 0.147.0 (`spawn_agent` validates `model` / `reasoning_effort`; the tool prompt says to set them only on user request, applicable AGENTS.md instructions, or a skill — rolepod's Codex AGENTS.md is that instruction), but full-history forks accept none and the V2 override picker is gated by `expose_spawn_agent_model_overrides`; a community thread (0.144.5+) reports the params ignored under MultiAgent V2 — role-file pins + the `[agents]` defaults are the reliable levers, the per-spawn `model` the tiered-by-task one.
...
- 6 core hooks (10 registrations): `preToolUse:Task` (dispatch-log — the reviewer dispatch-proof line for the commit gate), `sessionStart` (project context loader + `cursor-<conversation_id>` session lock), `preToolUse:Write|Edit|MultiEdit` (sweep-nudge edit flag; gate-reminder is silent there — the v2.130.2 write-time deny was measured to push the model into writing the file through the shell, so since v2.134.1 the commit gate is the only mechanical stop, as on Claude), `postToolUse:Write|Edit|MultiEdit` (gate-reminder soft reminders — schema-bound file written / high-risk path edited — as `additional_context`, because Cursor feeds `agent_message` to the model only on deny), `postToolUse:Read|Grep|Glob|WebFetch|WebSearch` + `afterShellExecution` + `beforeSubmitPrompt` (sweep-nudge: the shared `hooks/sweep-nudge.sh` behind a translator — Read bytes come from `content_length`, shell output is counted silently and delivered on the next read), `beforeShellExecution` on any `git` command (precommit-gate — since v2.134.0 a translator around the shared `scripts/shared/precommit-gate.sh`, so Cursor commits get the same evidence-based tiering as Claude instead of the old "any staged high-risk path → block"), `stop` (stop-unlock). Not portable on Cursor: fix-loop-breaker (no exit code on `afterShellExecution`), push-ref-check (no informational channel before a shell command), claim-verify-nudge (`beforeSubmitPrompt` cannot inject context). Hook JSON I/O follows [cursor.com/docs/hooks](https://cursor.com/docs/hooks): stdin JSON with `hook_event_name`/`tool_name`/`tool_input`/`command`/`workspace_roots` (no `cwd`), stdout JSON with `permission`/`user_message`/`agent_message`/`additional_context`. Exit code 2 = deny. Hook commands run with cwd = plugin root (`./scripts/x.sh` resolves).
- Skill frontmatter is intentionally stripped to `name` + `description` only (the two fields Cursor documents). Claude-specific keys (`tier`, `phase`, `when_to_use`, `disable-model-invocation`) are dropped at render time to avoid gambling on tolerance for unknown fields. Caveat: the `rolepod-full` and `deepen-codebase` commands lose their `disable-model-invocation: true` guard — its description ("Use only when the user explicitly invokes `/rolepod-full` ...") is phrased to keep auto-trigger rare.
...
Installs:
- `~/.gemini/extensions/rolepod/` (full extension: `GEMINI.md` context file, 15 agents in `agents/`, 12 skills, 5 hooks)
