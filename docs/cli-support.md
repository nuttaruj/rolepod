...
| Skills (`<plugin>/skills/<name>/SKILL.md`) | 13 — Core 10 + 2 commands + 1 on-demand (native) | 13 — Core 10 + 2 commands + 1 on-demand (native) | 13 — Core 10 + 2 commands + 1 on-demand (native) | 13 — Core 10 + 2 commands + 1 on-demand (native; frontmatter stripped to `name` + `description` per Cursor spec) | 13 — Core 10 + 2 commands + 1 on-demand (native) | 13 — Core 10 + 2 commands + 1 on-demand (native `SKILL.md`) |
...
| Codex CLI   | ✓ | ✓ | ✓ native — hooks fire without any opt-in on current Codex; `codex features list` (0.144.1, 2026-07-30) shows `hooks stable true` and the legacy `plugin_hooks` flag `removed` | ✓ verified (15 agents + 13 skills via native loader) | **Production** |
| Gemini CLI  | ✓ | ✓ | ✓ verified (SessionStart hook fires) | ✓ verified (13 skills enumerated) | **Production** |
...
Installs:
- `~/.gemini/extensions/rolepod/` (full extension: `GEMINI.md` context file, 15 agents in `agents/`, 13 skills, 5 hooks)
