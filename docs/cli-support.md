...
| Bash-write-is-an-edit (`block-subagent-commit.sh` write rule, 2026-09-18) | full — PreToolUse `Bash`: ledger row for Lead + sub-agent, write-scope class deny for a sub-agent (`subagent-write-scope.sh` bundled) | partial — PreToolUse `Bash`: same script runs, ledger row lands, but `subagent-write-scope.sh` is not bundled in the Codex plugin (it gates Claude's Edit/Write/MultiEdit/NotebookEdit, which Codex has none of) — no write-scope class deny | doctrine only — no PreToolUse Bash pre-hook | doctrine only — `beforeShellExecution` has no informational/deny channel for this | doctrine only — accepts no context field on any hook | doctrine only — `tool.execute.before` for Bash is not wired to this rule yet |
