...
- **Byte cap, not line cap.** Measured with every `{{INCLUDE:}}` expanded, since that is what the model reads: a phase skill ≤ 13 KB (`using-rolepod` ≤ 21.5 KB, `review-code` ≤ 19 KB, all SKILL.md ≤ 131 KB as a backstop — the per-skill caps do the work); supporting files ≤ 34 KB per skill, ≤ 176 KB total; no prose line past 600 chars. The old 190-line cap was met by lengthening lines while bytes grew 64% in three months. Caps sit ~1-2% above current size on purpose: when one bites, dedupe first, then move load-on-demand detail into `references/`, then name in the PR what the new doctrine replaces. Fallback sections stay concise; deep playbooks belong in agents.
