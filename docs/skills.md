...
Source of truth: [`core/fragments/skill-index-lean.md`](../core/fragments/skill-index-lean.md) (auto-generated by `build/render.sh`).
...
3. `bash build/render.sh` — regenerates `core/fragments/skill-index-lean.md`.
...
- **Each core skill is standalone.** It includes an agent-available path and a no-agent fallback so a copy-only install still works, and it is complete as ONE file: a runtime that ships only SKILL.md (no `templates/` `references/` `examples/`) still runs the whole workflow — the artifact line IS the template (it names every section in the template's words; the file only adds the layout).
