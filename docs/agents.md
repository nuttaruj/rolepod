...
This doc is the **reference**. Entry docs (`CLAUDE.md` / `AGENTS.md`) embed only a 1-line pointer so per-turn context stays small.
...
Source of truth: [`core/fragments/agent-roster-lean.md`](../core/fragments/agent-roster-lean.md) (auto-generated by `build/render.sh`).
...
3. Add `adapters/codex/agent-frontmatter/<name>.yml` (`tier:` + `model_reasoning_effort` + `sandbox_mode`). Add the agent's row to the "Default agent → tier mapping" table in `model-tier-policy.md` so the static gate can verify it.
4. Update the domain map above + `write-plan` agent-routing guidance.
5. `bash build/render.sh` — regenerates `agent-roster-lean.md` + per-CLI agent files (Codex `.toml` generated from the core body, no hand-maintained copy).
