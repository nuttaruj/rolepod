...
2. Add `adapters/claude/agent-frontmatter/<name>.yml` with `tier:` (cheap / balanced / strong) + `effort:` + `memory: project` (every role but `scout`, which carries no `memory:` — `memory:` auto-grants Write / Edit on Claude, and `scout` must stay read-only) + `tools:` including `Skill` (every role but `scout`). No `skills:` preload — a Claude role calls `Skill` on demand instead. The concrete model comes from `TIER_MODELS` in `build/merge-agent.py` — do NOT put a model name in the overlay, and no `maxTurns`: the brief carries the turn budget, the role file none (v2.119.0 — a cap measured as 12 of 105 reviewer runs paid for the read and dropped the report).
