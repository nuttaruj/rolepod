...
- **Hooks** → `hooks/hooks.json` registers 3 core hooks across SessionStart + PreToolUse (project context loader, schema-bound gate reminder, pre-commit test gate). Hooks require `codex features enable plugin_hooks` (default `under development, false`).
...
- SessionStart → project context loader
- PreToolUse apply_patch → schema-bound gate reminder (RED-test + reviewer-floor on high-risk paths)
- PreToolUse Bash → pre-commit test gate
