...
generate_skill_index_lean() {
  ...
  {
    echo "<!-- Auto-generated by build/render.sh — lean view: Tier 0 router + Tier 1 workflow + command alias. -->"
...
# Entry docs embed this instead of a full 15-row table. The shipped agent
# files are the catalog — each carries its own description.

generate_agent_roster_lean() {
  local out="$REPO_DIR/core/fragments/agent-roster-lean.md"
  {
    echo "<!-- Auto-generated by build/render.sh — lean view. The 15 agent files that ship ARE the full catalog. -->"
    echo ""
    echo "**15 specialists** organized by domain (backend / frontend / mobile / billing / ai-ml / data / qa / security / performance / architecture / design / ops / content / universal-review / scout). Lead doesn't pick from a list — \`write-plan\` maps path + concern + risk → agent when delegation helps. Each specialist ships as its own agent file — read its description for scope and boundaries; that is the catalog."
  } > "$out"
}
...
#   plugin/agents/<name>.md     (15 agents, gemini md+frontmatter shape)
